...
- **app.py** — `CurriculumApp(App)`: the main Textual application. Holds app-level reactive state (`mode`, `context`, `active_curriculum`, `active_topic`) and pushes the `ChatScreen` on mount.
- **state.py** — Shared types: `Mode` enum (IDLE, LEARN, REVIEW) and `ChatMessage` dataclass.
