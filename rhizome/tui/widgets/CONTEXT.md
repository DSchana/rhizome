...
- **status_bar.py** — `StatusBar(Static)`: persistent bar at the bottom of the chat screen with reactive `mode`, `context`, and `token_usage` (a `TokenUsageData` instance) properties. The token count is right-aligned and displayed as a formatted number when non-zero; if the model's context window limit is known, a usage percentage is appended (e.g. `tokens: 1,234 (2.3%)`).
