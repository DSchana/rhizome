...
All 8 model classes (`Base`, `Curriculum`, `CurriculumTopic`, `Topic`, `KnowledgeEntry`, `Tag`, `KnowledgeEntryTag`, `RelatedKnowledgeEntries`), plus `get_engine`, `get_session_factory`, and `init_db`. Import from `rhizome.db` directly.
