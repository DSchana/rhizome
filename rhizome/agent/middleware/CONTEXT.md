...
- **disable_parallel_tools.py** — `DisableParallelToolCallsMiddleware`, injects `parallel_tool_calls=False` into `model_settings` on every request. Prevents LangGraph's `ToolNode` from dispatching multiple tool calls concurrently via `asyncio.gather`, which would race on the shared `AsyncSession`.
