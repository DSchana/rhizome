...
- **review.py** — `build_review_tools(session_factory, scorer)` — consolidated review session tools: `review_get_past_sessions`, `review_show_session_state`, `review_update_session_state` (lazily initializes session; updates scope, config, flashcard queue, and plan ad-hoc), `review_record_interaction` (conversational review only), `review_present_flashcards` (interrupt-based flashcard presentation for both critique-during and critique-after modes; auto-scored cards are scored inline by the scorer subagent), `review_finish_session` (computes stats, builds and persists summary, clears state). Defines `ReviewConfigUpdate`, `ReviewFlashcardUpdate` Pydantic schemas and `AUTO_SCORE` constant. Phase progression is not tracked — guided by the system prompt.
