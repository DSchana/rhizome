...
- **review.py** — `build_review_tools(session_factory)` — 11 tools for the review session state machine covering SCOPING, CONFIGURING, PLANNING, REVIEWING, and SUMMARIZING phases. Tools that mutate `ReviewState` return `Command(update={"review": ...})`.
