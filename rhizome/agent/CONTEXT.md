...
- **state.py** — `RhizomeAgentState` TypedDict extending `AgentState` with `mode: str`, `review: ReviewState | None`, `flashcard_proposal_state: FlashcardProposalState | None` (consolidated proposal items, validation ID, and attempt counter), `commit_payload: list[dict] | None`, and `commit_proposal: list[CommitProposalEntry] | None` fields for checkpoint/replay support. Also defines `CommitProposalEntry` TypedDict.
...
Flashcard Proposals: `create_flashcard_proposal`, `validate_flashcard_proposal`, `present_flashcard_proposal`, `accept_flashcard_proposal`
