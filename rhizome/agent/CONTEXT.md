...
- **commit.py** — Commit subagent for extracting knowledge entries from selected conversation messages. Defines `CommitProposalResponseSchema` (containing `KnowledgeEntryProposalSchema` entries). `build_commit_subagent()` creates a `StructuredSubagent` with DATABASE tools and no user-settings middleware. `build_commit_subagent_tools()` returns three tools for the root agent: `invoke_commit_subagent` (sends messages to the subagent), `present_commit_proposal` (displays proposal via interrupt), and `accept_commit_proposal` (writes to DB).
