...
- **state.py** — `RhizomeAgentState` TypedDict extending `AgentState` with `mode: str`, `review: ReviewState | None`, `flashcard_proposal_state: FlashcardProposalState | None` (staged proposal items), and `commit_proposal_state: CommitProposalState | None` (payload, proposal entries, and diff summary) fields for checkpoint/replay support. Also defines `CommitProposalEntry` and `CommitProposalState` TypedDicts.
