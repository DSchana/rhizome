...
- **state.py** — `RhizomeAgentState` TypedDict extending `AgentState` with `mode: str`, `review: ReviewState | None`, `flashcard_proposal_state: FlashcardProposalState | None` (staged proposal items), `commit_payload: list[dict] | None`, and `commit_proposal: list[CommitProposalEntry] | None` fields for checkpoint/replay support. Also defines `CommitProposalEntry` TypedDict.
