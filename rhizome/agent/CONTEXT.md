...
- **state.py** — `RhizomeAgentState` TypedDict extending `AgentState` with `mode: str`, `review: ReviewState | None`, `flashcard_proposal_state: FlashcardProposalState | None` (consolidated proposal items and validation attempt counter), `commit_payload: list[dict] | None`, and `commit_proposal: list[CommitProposalEntry] | None` fields for checkpoint/replay support. Also defines `CommitProposalEntry` TypedDict.
...
Flashcard Proposals: `create_flashcard_proposal` (with `validate=True` for inline validation), `present_flashcard_proposal`, `accept_flashcard_proposal`
Commit: `inspect_commit_payload`, `invoke_commit_subagent`, `create_commit_proposal`, `present_commit_proposal`, `accept_commit_proposal`
